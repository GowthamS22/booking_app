...
class _CustomerScreenState extends State<CustomerScreen> {
          ...
          Column(
            ...
            children: [
              ...
              Row(
                ...
                children: [
                  Text('Players',style: TextStyle(fontSize: 18*ffem,fontWeight: FontWeight.bold,color: Palette.primaryColor),),
                  SizedBox(width: 100,),
                  Container(
        ...
        Container(
          ...
          child: StreamBuilder<QuerySnapshot>(
            ...
            builder: (context, snapshot) {
              ...
              customerController.users = snapshot.data!.docs.map((doc) {
              ...
              var _showList = _searchController.text.isNotEmpty ? customerController.filteredUsers : customerController.users;
              ...
              Container(
                child: SingleChildScrollView(
                  ...
                  child: DataTable(
                    ...
                    rows: _showList.map((user) => DataRow(
                      cells: [
                        DataCell(
                          Row(
                            children: [
                              ...
                              Text('${user.firstName} ${(user.lastName!=null ? user.lastName : '')}',style: TextStyle(fontSize: 15*ffem),),
                            ],
                          ),
                        ),
                        DataCell(
                          Text('${user.mobile}',style: TextStyle(fontSize: 15*ffem),),
                        ),
                        DataCell(
                            Text('${DateFormat('dd-MMM-yyyy').format(DateTime(user.createdAt!.year,user.createdAt!.month,user.createdAt!.day))}',style: TextStyle(fontSize: 15*ffem),)
                        ),
                        DataCell(
                          Row(
                            children: [
                              IconButton(
                                onPressed: () {
                                  ...
                                  controller.getBookingSlotDetails(userId: user.id, bookingId: null,subBookingId: null,selDate: controller.selectedDate);
                                  controller.getCurrentMembership(userId: user.id);
                                      ...
                                      children: [
                                        ...
                                        Container(
                                          ...
                                          child: Column(
                                            ...
                                            crossAxisAlignment: CrossAxisAlignment.start,
                                            children: [
                                              Text('Membership ',style: TextStyle(fontSize: 15*ffem,fontWeight: FontWeight.bold),),
                                              SizedBox(height: 5,),
                                              Row(
                                                mainAxisAlignment: MainAxisAlignment.start,
                                                crossAxisAlignment: CrossAxisAlignment.start,
                                                children: [
                                                  Text('${controller.currentPlan[0]['plan']} - ${NumberFormat.currency(locale: 'en_US', symbol: '\$').format(controller.currentPlan[0]['price'])}',style: TextStyle(fontWeight: FontWeight.bold,fontSize: 15*ffem),),
                                                  Text('  ${controller.currentPlan[0]['status']}',style: TextStyle(color: controller.currentPlan[0]['status']=='Active' ? Colors.green : Colors.red,fontWeight: FontWeight.bold,fontSize: 15*ffem),)
                                                ],
                                              ),
                                              SizedBox(height: 5,),
                                              Text('Expiry Date : ${controller.currentPlan[0]['remainingDays']}',style: TextStyle(fontSize: 15*ffem,fontWeight: FontWeight.bold),),
                                        ...
                                        ElevatedButton(
                                          onPressed: () {
                                            buildMembershipDialog(controller,user.id);
                                          },
                                          child: Text('Become a Member',style: TextStyle(fontSize: 15*ffem),),
                                          style: ButtonStyle(
                                              backgroundColor: MaterialStatePropertyAll(Palette.payColors),
                                              padding: MaterialStatePropertyAll(EdgeInsets.symmetric(vertical: 10*ffem,horizontal: 15*ffem))
                                          ),
                                        )
                                      ],
                                    ),
  ...
  buildCancelSlotPopup(CustomerController controller, List<String>? selectedIds) {
              ...
              content: Container(
                ...
                child: SingleChildScrollView(
                  child: Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                      Text('Membership Plans',style: TextStyle(fontSize: 15*ffem,color: Palette.primaryColor),),
                      Divider(height: 40,color: Palette.lightGrey,thickness: 2),
                      ...
                      ListView.builder(
                        ...
                        itemBuilder: (context, index) {
                          ...
                          return Container(
                            padding: EdgeInsets.symmetric(vertical: 15*ffem,horizontal: 15*ffem),
                            decoration: BoxDecoration(
                            ...
                            child: ListTile(
                              leading: Icon(Icons.star,size: 30*ffem,color: Palette.primaryColor), // Replace with your desired icon
                              title: Text('${membership.name} - ${membership.validDays} Days - ${NumberFormat.currency(locale: 'en_US', symbol: '\$').format(membership.price)}',style: TextStyle(fontSize: 15*ffem),),
                              trailing: Column(
                                ...
                                children: [
                                  ...
                                  ElevatedButton(
                                    onPressed: () {
                                      ...
                                      controller.buyMembership(userId: userId, membershipPlanId: membership.id, name: membership.name, price: membership.price,paymentType: '').then((value) {
                                        setState(() {
                                          controller.buyNowLoading.value = false;
                                        });
                                        Get.to(CheckoutScreen(type: 'Membership',));
                                      },);
                                    },
                                    child: Text("Buy Now",style: TextStyle(fontSize: 15*ffem),),
                                    style: ButtonStyle(
                                        shape: MaterialStatePropertyAll(RoundedRectangleBorder(borderRadius: BorderRadius.circular(20*ffem))),
                                        backgroundColor: MaterialStatePropertyAll(Palette.payColors),
                                        padding: MaterialStatePropertyAll(EdgeInsets.symmetric(vertical: 10*ffem,horizontal: 15*ffem))
                                    ),
                                  )
                              ...
                              subtitle: Container( // Use a container for flexible description
                                child: Text("Discount : ${membership.discount}%",style: TextStyle(fontSize: 15*ffem),),
                                padding: EdgeInsets.only(top: 8), // Add some padding for spacing
                              ),
                            ),
                          );
                        },
                          ...
                          :
                      Container(
                         child: Text('There is no Membership Plans Available..',style: TextStyle(fontSize: 15*ffem),),
                      )
