import 'package:badminton_app/app/getx_binding.dart';
...
class CheckoutController extends GetxController {
  ...
  //On click Pay now
  Future<void> processFinalCheckout({String? userId, String? name, String? mobile, String? email, String? notes, String? promoCode, String? paymentType, double? paid, double? balance}) async {
    try {

      //Validation
      Future<bool> validation = bulkValidateSlots(selectedBSlots: newBookingController.cartItems);
      ...
      if(await validation==false) {
        ...
        await docRef.set({
          ...
          'paymentStatus': (grandtotalPrice <= paid!) ? 'Paid' : 'Partially',
          ...
          var paymentdocRef = FirebaseFirestore.instance
              .collection(authController.centerSlug.toString())
              .doc('bookingPayments')
          ...
          for (var slot in newBookingController.cartItems) {
            ...
            DocumentReference slotPaymentRef = await FirebaseFirestore.instance
                .collection(authController.centerSlug.toString())
  ...
  double get membershipAmount {
    double val = customerController.selectedPlan.length > 0 ? double.parse(customerController.selectedPlan[0]['price'].toString()) : 0 ;
    return val;
  }
  
  double get totalAmount {
    double val = newBookingController.cartItems.fold(0, (double sum, BookingSlot bookingSlot) {
...
//     return val + membershipAmount;
  ...
  Future<void> productsPayment({
   ...
   await   FirebaseFirestore.instance
          ...
          .add({
        'bookingId': shoppingController.uuid.value, // Use the obtained bookingId
        'paymentType': paymentType,
  ...
  void printReceipt({List<BookingSlot>? bookingSlotItems ,}) async {
    ...
    print('checking the printer inside ${shoppingController.productsCartModal.length}');
    ...
    List<BookingSlot> mergedSlots = bookingSlotItems.length > 0 ? mergeBookingSlots(listItems) : [] ;
    if(bookingSlotItems.length > 0 ) {
      mergedSlots.removeWhere((bookingSlot) => (bookingSlot.slotType=='Repeat-Item'));
    }

    double bookingTotal = bookingSlotItems.length > 0 ? bookingSlotItems.fold(0, (double sum, BookingSlot bookingSlot) {
      return sum + (bookingSlot.price ?? 0);
    }) : 0 ;
   ...
   await docRef.get().then((value) {
    ...
    for(var printerIPs in settingController.printerList) {
      ...
      if(shoppingController.productsCartModal.length> 0){
        ...
        printer.text('Products#', styles: PosStyles(align: PosAlign.right),linesAfter: 1);
      }
      ...
      printer.row([
        ...
        PosColumn(
          text: '${NumberFormat.currency(locale: 'en_US', symbol: '\$').format(bookingTotal + shoppingController.totalPrice!.value)}',
          width: 3,
          styles: PosStyles(align: PosAlign.center, underline: false),
        ),
      ]);
  ...
  Future<void> makeMembershipPayment({
    ...
    try {
      ...
      showCustomSnackbar('Success', 'Membership Created Successfully', Colors.green);
