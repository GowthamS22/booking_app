...
class DefaultController extends GetxController {
  ...
  final List<String> dateOptions = ['Today', 'Yesterday', 'Tomorrow', 'Last 7 Days', 'Select Date'];
  ...
  Stream<QuerySnapshot> _allbookingSlotStream(DateTime selectedDate) {
    if(allselectedDateOption.value=='Last 7 Days') {
      return FirebaseFirestore.instance
          ...
          .where('date', isLessThanOrEqualTo: DateTime(DateTime.now().year,DateTime.now().month,DateTime.now().day))
      ...
      return FirebaseFirestore.instance
          ...
          .doc('bookingSlots')
          .collection('bookingSlot')
          .where('date', isEqualTo: DateTime(selectedDate.year,selectedDate.month,selectedDate.day))
          .where('status', isEqualTo: 'Booked')
          .snapshots();
    }
  ...
  String getServiceIconById(String serviceId) {
    final service = serviceList.firstWhere((element) => element['id'] == serviceId, orElse: () => null);
    if (service != null) {
      return '${service['name']}%${service['icon']}' as String;
    } else {
      return 'Service Not Found';
    }
  }
  ...
  void getUpcomingBookingSlots() async {
    ...
    QuerySnapshot bookingSlotsSnapshot = await FirebaseFirestore.instance
        ...
        .where('status',isEqualTo: 'Booked')
        .where('startTime',isLessThanOrEqualTo: DateTime(DateTime.now().year,DateTime.now().month,DateTime.now().day+1,DateTime.now().hour,DateTime.now().minute))
    ...
    switch (index) {
      case 0:
        filteredNewBookingSlots = newBookingSlots.where((slot) {
          ...
          return bookingId.contains(query.toLowerCase()) || court.contains(query.toLowerCase()) || name.contains(query.toLowerCase()) || mobile.contains(query.toLowerCase());
        }).toList();
        break;

      case 1:
        filteredFinishedUnpaidBookingSlots = finishedUnpaidBookingSlots.where((slot) {
          final bookingId = slot.bookingId!.toLowerCase();
          final court     = slot.court!.toLowerCase();
          final name     = slot.name!.toLowerCase();
          final mobile     = slot.mobile!.toLowerCase();
      ...
      case 2:
        filteredAllBookingSlots = allBookingSlots.where((slot) {
          final bookingId = slot.bookingId!.toLowerCase();
          final court     = slot.court!.toLowerCase();
          final name     = slot.name!.toLowerCase();
          final mobile     = slot.mobile!.toLowerCase();
  ...
  void onSortBookingSlotColumn1(int columnIndex, bool ascending) {
    if (columnIndex == 0) {
      ...
      newBookingSlots.sort((item1, item2) => compareString(ascending, item1.name, item2.name));
      filteredNewBookingSlots.sort((item1, item2) => compareString(ascending, item1.name, item2.name));
  ...
  void onSortBookingSlotColumn2(int columnIndex, bool ascending) {
    if (columnIndex == 0) {
      finishedUnpaidBookingSlots.sort((item1, item2) => compareString(ascending, item1.bookingId, item2.bookingId));
      filteredFinishedUnpaidBookingSlots.sort((item1, item2) => compareString(ascending, item1.bookingId, item2.bookingId));
    } else if (columnIndex == 1) {
      finishedUnpaidBookingSlots.sort((item1, item2) => compareString(ascending, item1.name, item2.name));
      filteredFinishedUnpaidBookingSlots.sort((item1, item2) => compareString(ascending, item1.name, item2.name));
    } else if (columnIndex == 2) {
      finishedUnpaidBookingSlots.sort((item1, item2) => compareString(ascending, item1.court, item2.court));
      filteredFinishedUnpaidBookingSlots.sort((item1, item2) => compareString(ascending, item1.court, item2.court));
    } else if (columnIndex == 3) {
      finishedUnpaidBookingSlots.sort((item1, item2) => compareString(ascending, item1.startTime, item2.startTime));
      filteredFinishedUnpaidBookingSlots.sort((item1, item2) => compareString(ascending, item1.startTime, item2.startTime));
    } else if (columnIndex == 5) {
      finishedUnpaidBookingSlots.sort((item1, item2) => compareString(ascending, item1.paymentStatus, item2.paymentStatus));
      filteredFinishedUnpaidBookingSlots.sort((item1, item2) => compareString(ascending, item1.paymentStatus, item2.paymentStatus));
    }
  ...
  void onSortBookingSlotColumn3(int columnIndex, bool ascending) {
    if (columnIndex == 0) {
      allBookingSlots.sort((item1, item2) => compareString(ascending, item1.bookingId, item2.bookingId));
      filteredAllBookingSlots.sort((item1, item2) => compareString(ascending, item1.bookingId, item2.bookingId));
    } else if (columnIndex == 1) {
      allBookingSlots.sort((item1, item2) => compareString(ascending, item1.name, item2.name));
      filteredAllBookingSlots.sort((item1, item2) => compareString(ascending, item1.name, item2.name));
    } else if (columnIndex == 2) {
      allBookingSlots.sort((item1, item2) => compareString(ascending, item1.court, item2.court));
      filteredAllBookingSlots.sort((item1, item2) => compareString(ascending, item1.court, item2.court));
    } else if (columnIndex == 3) {
      allBookingSlots.sort((item1, item2) => compareString(ascending, item1.startTime, item2.startTime));
      filteredAllBookingSlots.sort((item1, item2) => compareString(ascending, item1.startTime, item2.startTime));
    } else if (columnIndex == 5) {
      allBookingSlots.sort((item1, item2) => compareString(ascending, item1.paymentStatus, item2.paymentStatus));
      filteredAllBookingSlots.sort((item1, item2) => compareString(ascending, item1.paymentStatus, item2.paymentStatus));
    }
  ...
  Future<void> getBookingSlotDetails({String? userId, String? bookingId,String? subBookingId,DateTime? selDate}) async {
    ...
    if (userSnapshot.exists) {
      ...
      selectedUser.add(User(
        ...
        dateOfBirth: userData!['dateOfBirth']!=null ? (userData!['dateOfBirth'] as Timestamp).toDate() : null,
        city: userData!['city'],
      ...
      if (selectedDateOption.value == 'Last 7 Days') {
        bookingSlotsSnapshot = await baseQuery
            .where('date', isGreaterThanOrEqualTo: DateTime(selectedDate.year,selectedDate.month,selectedDate.day))
            .where('date', isLessThanOrEqualTo: DateTime(DateTime.now().year,DateTime.now().month,DateTime.now().day))
            .get();
      } else {
  ...
  Future<void> cancelBookingSlotsHold({List<String>? selectedIds}) async {
    try {
      ...
      if(totalSelectedItems==totalAvailableItems) {
        ...
        //Status alert
        showCustomSnackbar('Warning', 'Some Bookings already Started Unable to Delete that.', Colors.orange);
        cancelSlotIsLoading.value = false;
        update();
        Get.back();
  ...
  void getBookingData() async {
    ...
    if (bookingSnapshot.docs.isNotEmpty) {
      ...
      for (var booking in bookingSnapshot.docs) {
        ...
        //Booking slots query
        QuerySnapshot bookingSlotSnapshot = await FirebaseFirestore.instance
            .collection(authController.centerSlug.toString())
            .doc('bookingSlots')
            .collection('bookingSlot')
  ...
  void searchBooking(String query) {
    filteredBookingList = bookingList.where((booking) {
      ...
      return id.contains(query.toLowerCase()) || name.contains(query.toLowerCase()) || paymentStatus.contains(query.toLowerCase());
    }).toList();
    update();
  }
  ...
  void onSortBookingColumn(int columnIndex, bool ascending) {
    if (columnIndex == 0) {
      bookingList.sort((item1, item2) => compareString(ascending, item1.id, item2.id));
      filteredBookingList.sort((item1, item2) => compareString(ascending, item1.id, item2.id));
    } else if (columnIndex == 1) {
      bookingList.sort((item1, item2) => compareString(ascending, item1.name, item2.name));
      filteredBookingList.sort((item1, item2) => compareString(ascending, item1.name, item2.name));
    } else if (columnIndex == 2) {
      bookingList.sort((item1, item2) => compareString(ascending, item1.createdAt, item2.createdAt));
      filteredBookingList.sort((item1, item2) => compareString(ascending, item1.createdAt, item2.createdAt));
    } else if (columnIndex == 4) {
      bookingList.sort((item1, item2) => compareString(ascending, item1.status, item2.status));
      filteredBookingList.sort((item1, item2) => compareString(ascending, item1.status, item2.status));
    } else if (columnIndex == 5) {
      bookingList.sort((item1, item2) => compareString(ascending, item1.paymentStatus, item2.paymentStatus));
      filteredBookingList.sort((item1, item2) => compareString(ascending, item1.paymentStatus, item2.paymentStatus));
    }
  ...
  void getUpcomingBookingData() async {
    ...
    if(bookingSlotsSnapshot.docs.isNotEmpty) {
      ...
      for(var booking in bookingSnapshot.docs) {
        ...
        //Store the Bookings
        upcomingBookingList.add(Booking(booking.id, booking['userId'], booking['name'], booking['mobile'], booking['email'], booking['notes'], booking['subTotal'].toDouble(), booking['discount'].toDouble(), booking['gst'].toDouble(), booking['total'].toDouble(), booking['paymentType'], booking['paymentStatus'], booking['status'],bookingSlots, booking['createdBy'], booking['updatedBy'], booking['createdAt'].toDate(), booking['updatedAt'].toDate()));
      }
    }
  ...
  Future<String> getUserName(String bookingId) async {
    ...
    if (userName == null) {
      ...
      await FirebaseFirestore.instance.collection(authController.centerSlug.toString()).doc('bookings').collection('booking').doc(bookingId).get();
      if(bookingDoc!='') {
        // User name not found in cache, fetch it from Firestore
        DocumentSnapshot userDoc =
        await FirebaseFirestore.instance.collection(authController.centerSlug.toString()).doc('userDetails').collection('user').doc(bookingDoc['userId']).get();
        userName = userDoc['firstName'].toString();
        prefs.setString('booking_user_$bookingId', userName!); // Cache the service name
      } else {
        userName = '';
      }
    }
  ...
  Future calculatePayments({String? bookingId}) async {
    try {
      ...
      QuerySnapshot bookingPaymentSnapshot = await FirebaseFirestore.instance
          .collection(authController.centerSlug.toString())
          .doc('bookingPayments')
