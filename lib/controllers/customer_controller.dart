...
class CustomerController extends GetxController {
  ...
  List<User> users          = []; // Populate this list with your user data
  ...
  void onSortCustomerColumn(int columnIndex, bool ascending) {
    if (columnIndex == 0) {
      users.sort((item1, item2) => compareString(ascending, item1.firstName, item2.firstName));
      filteredUsers.sort((item1, item2) => compareString(ascending, item1.firstName, item2.firstName));
    } else if (columnIndex == 1) {
      users.sort((item1, item2) => compareString(ascending, item1.mobile, item2.mobile));
      filteredUsers.sort((item1, item2) => compareString(ascending, item1.mobile, item2.mobile));
    }
  ...
  Future<void> buyMembership({String? userId, String? membershipPlanId, String? name, double? price, String? paymentType}) async {
    try {
  ...
  Future<void> getCurrentMembership({String? userId}) async {
    isLoading.value = true;
    currentPlan.clear();
    QuerySnapshot userMembershipSnapshot = await FirebaseFirestore.instance
        .collection(authController.centerSlug.toString())
        .doc('userMemberships')
        .collection('userMembership')
    ...
    if(userMembershipSnapshot.docs.isNotEmpty) {
      ...
      DateTime endDate     = created.add(Duration(days: membershipSnapshot['validDays']));
